	...
	public render() {
		return (
			<>
				...
				<p>
					Here's a reliable way to learn something properly: be confidently wrong about it, in
					public, at scale.
				</p>
				...
				<p>
					...
					— the <code>exports</code> block and the handful of fields around it — with the caption
					"It is <i>crazy</i> how ALL of this is needed to just publish a package in today's JS
					ecosystem…"
				</p>

				<p>
					It did numbers. 697k views, a stack of replies all agreeing that yeah, this is too much
					just to ship a package. Which felt good, in a misery-loves-company way. One problem: the{' '}
					<code>package.json</code> I'd held up as the example of how-much-you-need wasn't even
					right.
				</p>

				<p>Then Andrew Branch replied.</p>

				<p>
					<ExternalLink href="https://twitter.com/atcb">Andrew</ExternalLink> works on TypeScript at
					Microsoft, on modules and auto-imports specifically. If <code>moduleResolution</code> has
					ever ruined your afternoon, he is somewhere upstream of that, in a good way. There is a
					real chance he understands Node's resolution algorithm more completely than any other
					living person. He was on holiday. He replied anyway, with{' '}
				...
				<p>
					What it does is the hard, abstract part. It models a module as a Module Record and pins
					down the lifecycle every graph goes through: parse, link, evaluate. The ordering modules
					run in. How cycles get resolved. How top-level <code>await</code> suspends a whole graph.
					What happens when something throws while the graph is still linking. That's the genuinely
				...
				<p>
					...
					package.json and never will. It pushes resolution back out to whoever embedded it: you
				...
				<p>
					...
					<code>'y'</code>" — is the runtime. Node, Bun, Deno, the browser. Everything you think of
					as "how imports work" lives down here, and not one bit of it is in the language:
				</p>
				...
				<p>
					...
					and the other runtimes copied it because what else were they going to do. So when people
					say "ESM vs CJS is a mess," what they're really describing is one specific layer — the
					runtime's resolution layer — being a mess, because it's the only layer on the hook for
					keeping both worlds alive at the same time.
				</p>

				<h3>Back to me being wrong</h3>

				<p>
					With that in hand, the mistake is easy to name. Here's roughly the shape of what I
					tweeted:
				</p>
				...
				<p>
					...
					walk me through slowly. A <code>.d.ts</code> describes the shape of a module <em>and</em>{' '}
					its format. An ESM <code>.d.ts</code> is describing a file with{' '}
					<code>export default</code>; a CJS <code>.d.cts</code> is describing one with{' '}
					<code>module.exports</code> / <code>export =</code>. Different things. Not swappable.
					Because our package.json says <code>"type": "module"</code>, a bare <code>.d.ts</code> is
					read as ESM and a <code>.d.cts</code> as CJS, exactly the way <code>.js</code> and{' '}
					<code>.cjs</code> split. So if you aim your <code>require</code> consumers at the ESM{' '}
					<code>.d.ts</code>, their compiler decides your CommonJS build has a default export that
					does not exist, and the errors show up three repos away in someone else's CI.
				</p>

				<p>
					The fix is exactly as dumb as it sounds: build your <code>.d.ts</code>, copy it, rename
					the copy to <code>.d.cts</code>, point <code>require</code> at the copy. That's it. That's
				...
				<p>
					Here's the bit that finally made the whole thing sit still in my head. TypeScript is
					running the runtime's resolver, statically, before anything executes. It has to. To
					type-check <code>import x from 'y'</code> it needs to know which file the runtime is going
					to choose, so it reimplements resolution at build time and guesses.{' '}
				...
				<p>
					...
					<code>.cjs</code> pair, the matching declarations, and the <code>exports</code> block. I
					have not hand-written that object since, and I'm not going to.
				</p>
				...
				<p>
					I left the tweet up. Wrong, with Andrew's reply underneath it doing the teaching. Felt
					more honest than quietly deleting and pretending.
				</p>
